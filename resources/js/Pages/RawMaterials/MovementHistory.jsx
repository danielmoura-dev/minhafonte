...
import { unitAbbr, formatQuantity, reasonLabel, REASON_LABELS } from '@/utils/rawMaterialUnits';
...
export default function MovementHistory({ movements, materials, suppliers, filters }) {
    const [f, setF] = useState({
        raw_material_id: filters.raw_material_id ?? '',
        type:            filters.type ?? '',
    ...
    function applyFilters(e) {
        e?.preventDefault();
        router.get(route('raw-materials.movements.history'), f, { preserveState: true, replace: true });
    }
    function clearFilters() {
        const empty = { raw_material_id: '', type: '', reason: '', supplier_id: '', date_from: '', date_to: '' };
        setF(empty);
        router.get(route('raw-materials.movements.history'), empty, { preserveState: true, replace: true });
    }
            ...
            <div className="flex items-center gap-4 mb-6">
                ...
                <div className="flex items-center gap-3">
                    ...
                    <div>
                        ...
                        <p className="text-sm text-gray-400">Todas as entradas e saídas de matéria-prima.</p>
                    </div>
                </div>
            </div>
            ...
            <form onSubmit={applyFilters} className="bg-white rounded-xl border border-gray-200 p-4 mb-5">
                <div className="flex flex-wrap items-end gap-3">
                    <div className="flex flex-col gap-1">
                        ...
                        <select value={f.raw_material_id} onChange={e => set('raw_material_id', e.target.value)} className={selectClass}>
                    ...
                    <table className="w-full text-sm whitespace-nowrap">
                        <thead>
                            <tr className="border-b border-gray-100 bg-gray-50">
                                ...
                                <th className="text-left px-4 py-3 font-semibold text-gray-500 text-xs uppercase tracking-wider">Matéria-prima</th>
                                ...
                                return (
                                    <tr key={mv.id} className="hover:bg-gray-50 transition">
                                        ...
                                        <td className="px-4 py-3 text-right font-medium text-gray-800">
                                            {mv.type === 'entrada' ? '+' : '−'}{formatQuantity(mv.quantity)} {ab}
                                        </td>
                                        <td className="px-4 py-3 text-right text-gray-500">{formatQuantity(mv.stock_before)} {ab}</td>
                                        <td className="px-4 py-3 text-right font-medium text-gray-700">{formatQuantity(mv.stock_after)} {ab}</td>
